...
contract NFTStack1155 is ERC1155, Ownable {
  string public constant name = "NFT Stack";
  ...
  function reserveNft() public onlyAuthorized {
    ...
    for (i = 0; i < reserveAtATime; i++) {
      emit AssetMinted(totalSupply + i, msg.sender);
      _mint(msg.sender, totalSupply + i, 1, "");
      reservedCount++;
    }

    totalSupply += reserveAtATime;
  }
  ...
  function reserveToCustomWallet(address _walletAddress, uint256 _count) public onlyAuthorized {
    ...
    for (i = 0; i < _count; i++) {
      emit AssetMinted(totalSupply + i, _walletAddress);
      _mint(_walletAddress, totalSupply + i, 1, "");
    }
  ...
  function mint(address _to, uint256 _count) public payable saleIsOpen {
    ...
    for (uint256 i = 0; i < _count; i++) {
      emit AssetMinted(totalSupply + i, _to);
